...
export class ProduceResourcesCommand extends Command<MyRoom> {
  execute() {
    const diceRoll = this.state.dice
    ...
    for (let i = 0; i < this.state.playerList.length; i++) {
      ...
      const player =
        this.state.playerList[
          (i + currentPlayerIndex) % this.state.playerList.length
        ];

      for (const hex of hexes) {
        const structures = this.state.structures.filter(
          (x) =>
            hex.intersections.includes(x.intersection) && x.owner === player.id
        );
        ...
        for (const structure of structures) {
          const resources =
            this.state.hexProductions
              .find(
                (x) =>
                  x.hexType === hex.type && x.structureType === structure.type
              )
