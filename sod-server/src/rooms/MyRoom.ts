...
export class MyRoom extends Room<GameState> {
      ...
      this.onMessage('startGame', (client, message) => {
        ...
        if (message?.autoPlace) {
          for (let i = 0; i < this.state.players.size * 2; i++) {
            const intersectionIndex = Math.floor(Math.random() * this.state.availableSettlementIntersections.length) - 1
            this.stateMachine.send({
              type: 'PLACE_SETTLEMENT',
              payload: {
                playerId: this.state.currentPlayer,
                intersectionId: this.state.availableSettlementIntersections[intersectionIndex]
              }
            })
  ...
  async onJoin(client: Client, options: RoomOptions) {
    ...
    console.log(`There are now ${this.state.players.size}/${this.options.numPlayers} players in the game.`)
  ...
  addPlayer(sessionId: string, name?: string, id?: string) {
    ...
    player.settlements.push(
      ...generate(this.options.numSettlements, (i) => Settlement.create(`${player.id}-${i}`, player.id))
    )
    player.cities.push(...generate(this.options.numCities, (i) => City.create(`${player.id}-${i}`, player.id)))
    player.roads.push(...generate(this.options.numRoads, (i) => Road.create(`${player.id}-${i}`, player.id)))
