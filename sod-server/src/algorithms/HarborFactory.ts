...
export class HarborFactory {
  ...
  createHarbors(gameState: GameState) {
    let coastalEdges = gameState.edges.filter((e) => e.getAdjacentHexes(gameState).length === 1)
    ...
    for (const [number, cardType, ratio] of harborPositionsDefault) {
      ...
      const harbor = new Harbor().assign({
        id: `harbor:${edge.id}`,
        edge: edge,
        hexPosition: edge.getAdjacentHexes(gameState)[0].getPosition(),
        cardTypes:
          cardType === 'Any'
            ? [
                ResourceCardVariants.Brick,
                ResourceCardVariants.Lumber,
                ResourceCardVariants.Ore,
                ResourceCardVariants.Grain,
                ResourceCardVariants.Wool
              ]
            : [cardType],
        ratio: ratio
      })
  ...
  orderEdgesLoop(gameState: GameState, edges: BorderEdge[]) {
    ...
    let start = edges.find(
      (e) => e.getConnectedEdges(gameState).length === 2 && Math.abs(e.pointA.y - e.pointB.y) < tolerance
    )
    if (!start) {
      start = edges.find(
        (e) => e.getConnectedEdges(gameState).length === 2 && Math.abs(e.pointA.x - e.pointB.x) < tolerance
      )
    }
    ...
    while (ordered.length < edges.length) {
      ...
      let candidates = last.getConnectedEdges(gameState).filter((e) => edgeIdSet.has(e.id) && !visited.has(e.id))
